...
class ArtemisPortPrinter extends ArtemisPortDevice implements IArtemisDevice {
  ...
  final ValueNotifier<DeviceConnectionStatus> _connectionStatus = ValueNotifier(DeviceConnectionStatus.disconnected);
  final ValueNotifier<String> _statusImagePathNotifier = ValueNotifier('assets/images/devices/notExist/BP.png');

  ArtemisPortPrinter({required super.portName, super.config, super.enableLogging = false}) {
  ...
  @override
  Future<bool> connect() => handler.open();
  ...
  void dispose() {
    ...
    handler.dispose(); // Use dispose to clean up hotplug timers
    super.dispose();
  }
  ...
  Widget icon([double size = 24]) {
    log("image of ${_statusImagePathNotifier.value}");
    return ValueListenableBuilder(valueListenable: _statusImagePathNotifier, builder: (BuildContext context, String value, Widget? child) {
      return Image.asset(
        value,
        width: size,
        package: 'artemis_port_util', // Assuming this is your package name
      );
    },);
  }
  ...
  void _updateStatus() {
    // log("update status to ${portStatus.value.name} --${handler.statusMgr.status.state.name}");
    final deviceStatus = handler.statusMgr.status;

    if (portStatus.value == PortStatus.open && deviceStatus.state == StatusState.busy) {
