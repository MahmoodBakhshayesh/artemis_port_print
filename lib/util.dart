...
//       final looksFramed = chunk.contains(stx) || chunk.contains(etx); // quick heuristic
//
//       if (looksFramed) {
...
//   Future<PrintResult> printText(String data, {bool addCRLF = true}) {
//     final send = addCRLF ? '$data\r\n' : data;
//     // Most printer protocols are ASCII; avoid multi-byte surprises
//     final bytes = Uint8List.fromList(ascii.encode(send));
...
//
//   /// Enqueue raw bytes; returns the classified result.
...
// //
// //   // if STX/ETX appear in raw, strip them
// //   final ResponseClassifier classify; //
// //   // serialize requests
// //   Future<void> _tail = Future<void>.value();
// //
// //   SerialPrintQueue(this.sp, {this.timeout = const Duration(seconds: 2), this.quietWindow = const Duration(milliseconds: 150), this.stripFraming = true, ResponseClassifier? classifier})
// //     : classify = classifier ?? _defaultClassifier;
// //
...
// //     }
// //
// //     sub = sp.onData.listen(
// //       (evt) {
// //         if (evt is DataReceive) {
// //           final payload = stripFraming ? _stripStxEtx(Uint8List.fromList(evt.bytes)) : Uint8List.fromList(evt.bytes);
// //           chunks.addAll(payload);
// //           quiet?.cancel();
// //           quiet = Timer(quietWindow, finish);
// //         }
